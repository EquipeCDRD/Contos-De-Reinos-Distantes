...
function validaDel(q) {
  ...
  if ($("input[name=txt" + q + "").val() != "") {
    if ($("textarea[name=txamotivo" + q + "]").val() != "") {
      conf = confirm(
        "Você tem certeza que deseja deletar o usuario? (um " + q + ")"
      );
    } else {
...
$(function() {
  //Esconde a div de editar notificacoes.
...
function validaNotificacao(compedit) {
  //Recebe o parâmetro se o foi chamado pelo compor ou pelo editar.
  var conf = false;
  if (compedit == 0) {
        ...
        if ($("input[name=txtapelidoadm]").val() != "") {
          if ($("input[name=pwdsenhaadm]").val() != "") {
            if ($("input[name=pwdconfsenhaadm]").val() != "") {
              if (
                $("input[name=pwdsenhaadm]").val() ==
                $("input[name=pwdconfsenhaadm]").val()
              ) {
                conf = confirm(
                  "Tem certeza que deseja cadastrar um novo administrador?"
                );
              } else {
