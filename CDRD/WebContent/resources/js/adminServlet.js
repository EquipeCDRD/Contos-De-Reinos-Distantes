		...
		.ready(
				function() {
					...
					insereNotificacao = function() {
						var notificacao = $("textarea[name=txacompnotificacao]")
								.val();
						$
								.ajax({
									type : "POST",
									...
									error : function(info) {
										alert("Erro ao cadastrar uma nova notificação: "
												+ info.status
												+ " - "
												+ info.statusText);
									}
					...
					alteraNotificacao = function() {
						var notificacao = $("textarea[name=txaeditnotificacao]")
								.val();
					...
					$(function buscaLogDeAcesso() {
						var html = "<table class='tabelaGeral'>" + "<tr>"
								+ "<th>Usuário:</th>"
								+ "<th>Data - Horário</th>" + "</tr>";
					...
					geraTabelaLogDeAcesso = function(listLog) {
						...
						if (listLog != undefined && listLog.length > 0) {
							for (var i = 0; i < listLog.length; i++) {
								dados += "<tr>" + "<td><span>"
										+ listLog[i].usuario + "</span></td>"
										+ "<td><span>" + listLog[i].dataCriacao
										+ "</span></td>" + "</tr>";
							}
						} else if (listNotificacao == "") {
					...
					busca = function(p, q) {
						var valorBusca = $("input[name=txtbusca" + q + "]")
								.val();
						var html;
						$
								.ajax({
					...
					deletaUsuario = function(p, q) {
						if (validaDel(q)) {
							...
							var motivo = $("textarea[name=txamotivo" + q + "]")
									.val();
							$.ajax({
								type : "POST",
					...
					cadastraAdm = function() {
						if (validaCadastroAdm() == true) {
							$.ajax({
								...
								error : function(info) {
									alert("Erro ao cadastrar um novo jogador: "
											+ info.status + " - "
											+ info.statusText);
								}
							});
					...
					$(function buscaPontuacoes() {
						var html = "<table class='tabelaGeral' id='tabelaRanking'>"
								+ "<!--Tabela de Ranking-->"
								+ "<tr>"
								+ "<th>Posição</th>"
								+ "<th>Pontuação</th>"
						...
						$
								.ajax({
									...
									data : (dados = "usuarioid=null&identificador=ranking"),
									success : function(dados) {
										html += geraTabelaRanking(dados);
										console.log(html);
										$("#containerRanking").html(html);
									},
									error : function(info) {
										html += "<tr>"
												+ "<td colspan='3'> Erro ao carregar o ranking: "
												+ info.status + " - "
												+ info.statusText + "</td>"
												+ "</tr>";
									}
...
geraTabelaRanking = function(listaRanking) {
	...
	if (listaRanking != undefined && listaRanking.length > 0) {
		for (var i = 0; i < listaRanking.length; i++) {
			dados += "<tr>" + "<td>" + listaRanking[i].posicaoRanking + "</td>"
					+ "<td>" + listaRanking[i].nomeDeUsuario + "</td>" + "<td>"
					+ listaRanking[i].score + "</td>" + "</tr>";
		}
	} else {
