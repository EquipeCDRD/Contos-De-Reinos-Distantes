 ...
 */
 ...
 $(document).ready(function(){
	...
	$(function(){
		var html = "<table id='tabela'> <!--Tabela de Ranking-->"
				 + "<tr>"
	...
	geraTabelaRanking = function(listaRanking){
		...
		if (listaRanking != undefined && listaRanking.length > 0){
			for (var i=0; i<listaRanking.length; i++){
				dados += "<tr>" +
						"<td class='tdTabela'><span>"+listaRanking[i].posicaoRanking+"</span></td>" +
						"<td class='tdTabela'><span>"+listaRanking[i].nomeDeUsuario	+"</span></td>" +
						"<td class='tdTabela'><span>"+listaRanking[i].score+"</span></td>" +
						"</tr>"
			}
	...
	cadastrar = function(){
		$.ajax({
			...
			error: function (info) {
				alert("Erro ao cadastrar novo jogador: "+ info.status + " - " + info.statusText);
			}
		});
	}
